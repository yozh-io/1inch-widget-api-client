...
export class InfoApi extends runtime.BaseAPI {
     ...
     */
    async chainProtocolControllerGetProtocolsImagesRaw(initOverrides?: RequestInit): Promise<runtime.ApiResponse<ProtocolsResponseDto>> {
        const queryParameters: any = {};

        const headerParameters: runtime.HTTPHeaders = {};

        const response = await this.request({
            path: `/v4.0/1/liquidity-sources`,
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        }, initOverrides);
     ...
     * List of liquidity sources that are available for routing in the 1inch Aggregation protocol
     */
    async chainProtocolControllerGetProtocolsImages(initOverrides?: RequestInit): Promise<ProtocolsResponseDto> {
        const response = await this.chainProtocolControllerGetProtocolsImagesRaw(initOverrides);
